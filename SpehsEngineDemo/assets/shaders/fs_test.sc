...
void main()
{
	...
	vec3 ambientColor = u_ambientLightColor * u_ambientLightIntensity * vec3(0.0, 1.0, 0.0);
	...
	for (int i = 0; i < u_pointLightCount; i++)
	{
		...
		diffuseColor = diffuseColor * vec3(0.0, 1.0, 0.0) + diffuseFactor * distanceFactor * u_pointLightColor(i);
		
		vec3 viewDirection = normalize(viewPosition - position);
		vec3 reflectDirection = reflect(-lightDirection, normal);
		float specularFactor = pow(max(0.0, dot(viewDirection, reflectDirection)), shininess);
		specularColor = specularColor * vec3(0.0, 1.0, 0.0) + specularFactor * specularStrength * distanceFactor * u_pointLightColor(i);
	}
